...
import jax
from cvae_flax import CVAE
from cvae_utils import extract_decoder, extract_pointnet
from locale import normalize
import numpy as np
import jax.numpy as jnp
from functools import partial
...
class batch_occ_tracking():
	...
	def __init__(self, P, Pdot, Pddot, v_max, a_max, t_fin, num, num_batch_projection,
					...
					d_min_tracking_vehicle, d_max_tracking_vehicle, package_path):
		...
		directory_z_dim_2 = package_path + "/nn_weights/cvae/z_dim_2.pth"
		directory_z_dim_6 = package_path + "/nn_weights/cvae/z_dim_6.pth"
		
		pointnet_Wandb_2, pointnet_BN_2 = extract_pointnet(2, directory_z_dim_2)
		decoder_Wandb_2, decoder_BN_2 = extract_decoder(2, directory_z_dim_2)
		pointnet_Wandb_6, pointnet_BN_6 = extract_pointnet(6, directory_z_dim_6)
		decoder_Wandb_6, decoder_BN_6 = extract_decoder(6,directory_z_dim_6)

		# No. of Generated Samples
		num_batch_z_dim_2 = 40
		num_batch_z_dim_8 = 40

		state_mean, state_std = -0.4467416132625483, 3.1826055232556145
		min_pcd, max_pcd = -62.39320591659159, 53.55664677091144

		# Initialize HMnet
		self.model_z_dim_2 = CVAE(num_batch_z_dim_2, jnp.asarray(P), jnp.asarray(Pdot), jnp.asarray(Pddot), 
					pointnet_Wandb_2, pointnet_BN_2, decoder_Wandb_2, decoder_BN_2,
					state_mean, state_std, min_pcd, max_pcd, zdim=2)

		self.model_z_dim_8 = CVAE(num_batch_z_dim_8, jnp.asarray(P), jnp.asarray(Pdot), jnp.asarray(Pddot), 
					pointnet_Wandb_6, pointnet_BN_6, decoder_Wandb_6, decoder_BN_6,
					state_mean, state_std, min_pcd, max_pcd, zdim=6)
	...
	@partial(jit, static_argnums=(0,))	
	def compute_initial_samples(self, drone_states, vehicle_states, pointcloud):

		
		state = jnp.hstack((drone_states, vehicle_states)).reshape(1, -1)
		pcd_data = np.transpose(pointcloud.reshape(1, 200, 2), axes=(0, 2, 1))
		
		y_star_z_dim_2 = self.model_z_dim_2.forward(state, pcd_data)
		y_star_z_dim_8 = self.model_z_dim_8.forward(state, pcd_data)

		y_star = jnp.vstack((y_star_z_dim_2, y_star_z_dim_8))

		c_x_samples_init_drone = y_star[:, 0:11]
		c_y_samples_init_drone = y_star[:, 11:22]

		c_x_samples_init_vehicle = y_star[:, 22:33]
		c_y_samples_init_vehicle = y_star[:, 33:44]

		x_samples_init_drone = jnp.dot(self.P_jax, c_x_samples_init_drone.T).T
		y_samples_init_drone = jnp.dot(self.P_jax, c_y_samples_init_drone.T).T

		xdot_samples_init_drone = jnp.dot(self.Pdot_jax, c_x_samples_init_drone.T).T
		ydot_samples_init_drone = jnp.dot(self.Pdot_jax, c_y_samples_init_drone.T).T

		xddot_samples_init_drone = jnp.dot(self.Pddot_jax, c_x_samples_init_drone.T).T
		yddot_samples_init_drone = jnp.dot(self.Pddot_jax, c_y_samples_init_drone.T).T


		x_samples_init_vehicle = jnp.dot(self.P_jax, c_x_samples_init_vehicle.T).T
		y_samples_init_vehicle = jnp.dot(self.P_jax, c_y_samples_init_vehicle.T).T

		xdot_samples_init_vehicle = jnp.dot(self.Pdot_jax, c_x_samples_init_vehicle.T).T
		ydot_samples_init_vehicle = jnp.dot(self.Pdot_jax, c_y_samples_init_vehicle.T).T

		xddot_samples_init_vehicle = jnp.dot(self.Pddot_jax, c_x_samples_init_vehicle.T).T
		yddot_samples_init_vehicle = jnp.dot(self.Pddot_jax, c_y_samples_init_vehicle.T).T

		

		return c_x_samples_init_drone, c_y_samples_init_drone, x_samples_init_drone, y_samples_init_drone,\
				 xdot_samples_init_drone, ydot_samples_init_drone, xddot_samples_init_drone, yddot_samples_init_drone,\
				 c_x_samples_init_vehicle, c_y_samples_init_vehicle, x_samples_init_vehicle, y_samples_init_vehicle,\
				 xdot_samples_init_vehicle, ydot_samples_init_vehicle, xddot_samples_init_vehicle, yddot_samples_init_vehicle
	...
	def compute_cost_batch(self, xddot_samples, yddot_samples, x_samples, y_samples,
		...
		# occlusion_cost *=  self.discouting_factor
