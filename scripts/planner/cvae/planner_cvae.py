...
import vehicle_module_cvae as vehicle_module
import drone_module_cvae as drone_module
import sys
import numpy as np
...
def pointcloudCallback(vehicle_msg):
    global x_obs_pointcloud_vehicle, y_obs_pointcloud_vehicle,\
            is_received, pointcloud_mutex, obstacle_points_vehicle, x_obs_down_sampled, y_obs_down_sampled, xyz,\
            odom_mutex
...
def mpc():
    ...
    v_max = 2
    a_max = 3
    num_batch_projection = 80

    rho_ineq = 1
    rho_projection = 1
    rho_tracking = 0.1
    rho_obs = 1
    maxiter_projection = 2
    maxiter_cem = 8
    maxiter_mpc = 15000
    occlusion_weight = 1

    ellite_num_projection =  80
    ellite_num = 20
    ...
    v_max_drone = 2
    a_max_drone = 3
    num_batch_projection_drone = 80

    rho_ineq_drone = 1
    rho_projection_drone = 1
    rho_tracking_drone = 0.8
    rho_obs_drone = 12
    maxiter_projection_drone = 2
    maxiter_cem_drone = 8
    occlusion_weight_drone = 1e+4

    ellite_num_projection_drone =  80
    ellite_num_drone = 20
    ...
    prob_drone = drone_module.batch_occ_tracking(P, Pdot, Pddot, v_max_drone, a_max_drone, t_fin, num, num_batch_projection_drone,
                                                ...
                                                initial_up_sampling_drone, target_distance_weight_drone, smoothness_weight_drone,
                                                 a_obs_drone, b_obs_drone, num_obs_drone, d_min_tracking_vehicle, d_max_tracking_vehicle, package_path)
    ...
    for i in range(0, maxiter_mpc):
        ...
        obstacles_for_planning = copy.deepcopy(obstacles)
        idxes = np.argwhere(obstacles[:,:]>=90)
        obstacles[idxes, 0] = obstacles[20, 0]
        obstacles[idxes, 1] = obstacles[20, 1]
    
        obstacle_for_init_jax = jnp.asarray(obstacles)
        obstacles_for_planning_jax = jnp.asarray(obstacles_for_planning)
        ...
        drone_pose_vel_temp  = np.copy(drone_pose_vel)
        vehicle_pose_vel_temp = np.copy(vehicle_pose_vel)
        ...
        vehicle_jax_obstacle_points_min = prob.get_closest_obstacles(obstacles_for_planning_jax, x_init_vehicle, y_init_vehicle)
        drone_jax_obstacle_points = prob_drone.get_closest_obstacles(obstacles_for_planning_jax, x_init_drone, y_init_drone)

        vec_vx = vehicle_pose_vel[4] * jnp.cos(vehicle_pose_vel[3]) - vehicle_pose_vel[5] * jnp.sin(vehicle_pose_vel[3])
        vec_vy = vehicle_pose_vel[4] * jnp.sin(vehicle_pose_vel[3]) + vehicle_pose_vel[5] * jnp.cos(vehicle_pose_vel[3])
        drone_states = jnp.hstack((drone_pose_vel_temp[0] , drone_pose_vel_temp[1], drone_pose_vel[4], drone_pose_vel[5]))
        vehicle_states = jnp.hstack([0, 0, vec_vx, vec_vy, x_tracking_drone[0,-1], y_tracking_drone[0,-1], vx, vy, x_rack_traj.flatten(), y_rack_traj.flatten()])
		...
		xdot_samples_init_vehicle, ydot_samples_init_vehicle, xddot_samples_init_vehicle, yddot_samples_init_vehicle  = prob_drone.compute_initial_samples(drone_states, vehicle_states, obstacle_for_init_jax)
...
if __name__ == "__main__":
    ...
    rack_tracking_marker = createLineMarker(color=[1, 0.487, 0.278, 0])
    ...
    drone_traj_curtailed_publisher = rospy.Publisher('/drone_traj_curtailed', Marker, queue_size=10)
    vehicle_traj_curtailed_publisher = rospy.Publisher('/vehicle_traj_curtailed', Marker, queue_size=10)
    ...
    rack_tracking_traj_publisher = rospy.Publisher('/rack_tracking_traj', Marker, queue_size=10)
