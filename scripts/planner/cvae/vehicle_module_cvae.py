...
class batch_occ_tracking():
	...
	def __init__(self, P, Pdot, Pddot, v_max, a_max, t_fin, num, num_batch_projection,
		...
		# eps_k_up_sampling = np.random.multivariate_normal(mu, 0.0009*cov, (int((self.num_batch_projection * self.initial_up_sampling)), ))
		...
		self.discouting_factor =1# jnp.tile(self.discouting_factor, (self.ellite_num_projection,1))
	...
	def initial_alpha_d_obs(self, x_samples_init, y_samples_init,
		...
		return alpha_tracking, d_tracking, alpha_obs, d_obs, alpha_v, d_v, alpha_a, d_a		
	...
	def compute_projection_samples(self, x_init, vx_init, ax_init, y_init, vy_init, ay_init, \
								x_tracking, y_tracking, lamda_x, lamda_y, x_samples_init, y_samples_init, 
                        			...
                        			xddot_samples_init, yddot_samples_init,
									vx_tracking, vy_tracking, x_obs_traj, y_obs_traj):

		b_x_eq, b_y_eq = self.compute_boundary_vec(x_init, vx_init, ax_init, y_init, vy_init, ay_init, vx_tracking, vy_tracking)

		alpha_tracking, d_tracking, alpha_obs, d_obs, alpha_v, d_v, alpha_a, d_a = self.initial_alpha_d_obs(x_samples_init, y_samples_init, 
																				xdot_samples_init, ydot_samples_init,\
                        														xddot_samples_init, yddot_samples_init, \
																				x_tracking, y_tracking, x_obs_traj, y_obs_traj)
		...
		for i in range(0, self.maxiter_projection):
			...
			alpha_tracking, d_tracking, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, \
				res_tracking_vec, res_acc_vec, res_vel_vec, res_tracking_vec, \
					 res_acc_norm, res_vel_norm, res_norm_batch, alpha_obs, d_obs, res_obs_norm = self.compute_alph_d(x_samples, y_samples, xdot_samples, ydot_samples,
	...
	def compute_cost_batch(self, xddot_samples, yddot_samples, x_samples, y_samples,
							 d_avg_tracking, x_tracking, y_tracking, obstacle_points, res_norm_projection): 

		mu =  2.3333
		std = 6.0117

		tiled_obstacle_points = jnp.tile(obstacle_points, (self.num * self.ellite_num_projection,1))
		tiled_tracking_trajectory_x = jnp.tile(x_tracking, (self.ellite_num_projection))
		tiled_tracking_trajectory_y = jnp.tile(y_tracking, (self.ellite_num_projection))
		...
		total_cost =  occlusion_cost * 1e3 +  5* cost_smoothness + 5 * res_norm_projection
	...
	def compute_shifted_samples(self, c_x_ellite_vehicle, c_y_ellite_vehicle, x_obs_traj, y_obs_traj):
		...
		c_ellite_shift_temp = jax.random.multivariate_normal(key, c_mean, c_cov, (self.initial_up_sampling*self.num_batch_projection, ))
		...
		wc_alpha_vehicle = wc_alpha_temp_vehicle.transpose(1, 0, 2)
		ws_alpha_vehicle = ws_alpha_temp_vehicle.transpose(1, 0, 2)
		wc_alpha_vehicle = wc_alpha_vehicle.reshape(self.num_batch_projection * self.initial_up_sampling, self.num * x_obs_traj.shape[0])
		ws_alpha_vehicle = ws_alpha_vehicle.reshape(self.num_batch_projection * self.initial_up_sampling, self.num * y_obs_traj.shape[0])

		dist_obs_vehicle = -wc_alpha_vehicle**2/(self.a_obs**2) - ws_alpha_vehicle**2/(self.b_obs**2)+ 1
		dist_obs = dist_obs_vehicle

		cost_obs_penalty = jnp.linalg.norm(jnp.maximum(jnp.zeros((self.initial_up_sampling * self.num_batch_projection, self.num * x_obs_traj.shape[0])), dist_obs), axis = 1)
		idx_ellite = jnp.argsort(cost_obs_penalty)
	...
	def compute_cem(self, key, x_init, vx_init, ax_init, y_init, vy_init, ay_init,
					 x_tracking, y_tracking, lamda_x, lamda_y, x_samples_init, y_samples_init, c_x_samples_init, c_y_samples_init,\
					   ...
					   d_avg_tracking, obstacle_pointcloud):
		...
		for i in range(0, self.maxiter_cem):
			...
			cost_batch = self.compute_cost_batch(xddot_ellite_projection, yddot_ellite_projection, x_ellite_projection, y_ellite_projection,
													 d_avg_tracking, x_tracking, y_tracking, obstacle_pointcloud, res_norm_ellite_projection)
        ...
        # x_shifted_samples = x_ellite_projection[0:num_samples_warm_start]
        # y_shifted_samples = x_ellite_projection[0:num_samples_warm_start]
	...
	def compute_controls(self, c_x_best, c_y_best, dt_up, vx_tracking, vy_tracking, 
		...
		# x_tracking_up = x_tracking_init #+ vx_tracking * tot_time_copy_up.flatten()
		# y_tracking_up = y_tracking_init #+ vy_tracking * tot_time_copy_up.flatten()

		# alpha_drone_temp = jnp.arctan2(y_tracking_up - y_up, x_tracking_up - x_up )
		...
		alphadot = 0#jnp.diff(jnp.hstack((alpha_init,  alpha_drone)) )/dt_up
		alphadot_drone = 0#jnp.mean(alphadot[0:num_average_samples])
	...
	@partial(jit, static_argnums = (0,) )
	def compute_controls_up_sampled(self, c_x_best, c_y_best, dt_up, vx_tracking, vy_tracking, 
							t_update, tot_time_copy_up, x_init, y_init, alpha_init,
                            x_tracking_init, y_tracking_init):
		...
		vx_control_local = vx_local
		vy_control_local = vy_local

		alphadot = 0#jnp.diff(jnp.hstack((alpha_init,  alpha_drone)) )/dt_up
		alphadot_drone = 0#jnp.mean(alphadot[0:num_average_samples])

		return vx_control_local, vy_control_local, ax_control, ay_control, \
						 alphadot_drone, x_up, y_up, vx_control, vy_control
